...
class RulerView: UIView {
  ...
  override func draw(_ rect: CGRect) {
    ...
    while Double(currentOffset) < limit {
      drawStick(count: &count, currentOffset: &currentOffset)
    }
  }

  private func drawStick(count: inout Int, currentOffset: inout CGFloat) {
    ...
    UIRectFill(
      CGRect(
        x: isVertical ? 0 : currentOffset,
        y: isVertical ? currentOffset : 0,
        width: width,
        height: height
      )
    )
